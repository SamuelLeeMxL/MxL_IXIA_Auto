...
log_filename = datetime.datetime.now().strftime(filename + '_CablePlug_%Y-%m-%d_%H_%M_%S.log')
formatter = logging.Formatter('%(asctime)s %(levelname)-5s %(message)s')
logger = logging.getLogger('CablePlug:')
...
class IXIA():
    def my_func(self, portspeed, mode):
        if mode == 'master':
            ...
            ixNet.setMultiAttribute( vport2 + "/l1Config/novusTenGigLan",
                    ...
                    '-speedAuto'                    ,[portspeed],)
            if len(IXIAport) == 4:
                ixNet.setMultiAttribute( vport3 + "/l1Config/novusTenGigLan",
                    '-autoInstrumentation'          ,'floating',
                    '-loopback'                     ,'false',
                    '-txIgnoreRxLinkFaults'         ,'false',
                    '-loopbackMode'                 ,'none',
                    ...
                    '-ppm'                          ,'0',
                    '-speedAuto'                    ,[portspeed],)
                ixNet.setMultiAttribute( vport4 + "/l1Config/novusTenGigLan",
                    '-autoInstrumentation'          ,'floating',
                    '-loopback'                     ,'false',
                    '-txIgnoreRxLinkFaults'         ,'false',
                    '-loopbackMode'                 ,'none',
    ...
    def my_func2(self, passtimes, failtimes, transmissionControl_duration, transmissionControl_type, frameSize_fixedSize, frameSize_type, frameRate_rate, frameRate_rate2, display):
        ...
        trafficItem = ixNet.getList(root + 'traffic', 'trafficItem')[0]
        if len(IXIAport) == 4:
            trafficItem2 = ixNet.getList(root + 'traffic', 'trafficItem')[1]
        ...
        while i < 1:
            ...
            logger.info ("Sleep 3 sec for protocols to start")
            time.sleep(3)
            ...
            logger.info ("Sleep 3 sec to send all traffic")
            time.sleep(3)
            ...
            time.sleep(transmissionControl_duration + 10)
            ...
            for txStat, rxStat in zip(txFrames, rxFrames):
                logger.info (PortName[j] + " Tx Frames %s ," % txStat + PortName2[j] + " Rx Frames %s" % rxStat)
                if txStat == "1000000":
                    if txStat != rxStat:
                        if (int(txStat) - int(rxStat)) > 3:
                    ...
                    if (1000000 - int(rxStat)) > 3:
                        logger.info ("P" + str(j + 1) + " Tx - Rx packet loss more than 3,Packet loss: " + str(1000000 - int(rxStat)))
                    if j == 0:
                        globals.num5 += 1
                        globals.str3 = "loss " + str(1000000 - int(rxStat))
                    elif j == 1:
                        globals.num6 += 1
                        globals.str4 = "loss " + str(1000000 - int(rxStat))
        ...
        if display == True:
            ...
            logger.info ("    =======  Link Up FAILED = %s  =======" % globals.num4)
            logger.info ("=================================================")
        else:
            logger.info ("=================================================")
            logger.info ("=================================================")
